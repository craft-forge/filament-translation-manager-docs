...
![Translation Manager for Filament](.github/translation-manager.jpg)
...
No custom themes, no published views, no complex setup. Just one migration (a single `translations` table) and a config file. Register the plugin and you're ready to go.
...
To avoid merge conflicts in `lang/` when working in a team, you can exclude translations from version control:
...
Translations are then managed via the UI and published to files; for deployment you can use CSV export/import or the Artisan commands (see below).
</details>
...
Click **Manage Packages** to open the package management panel. Here you can:
...
The plugin automatically creates a backup before **Publish** — the operation that writes translations from the database to your `lang/` files. This ensures you can always restore your translations if needed.

You can restore any previous backup from the **CSV → Backups** menu. Note: restoring a backup will replace all current translations in the database.
